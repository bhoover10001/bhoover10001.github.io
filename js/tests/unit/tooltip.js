$(function () {
    ...
    module("tooltip")
      ...
      test("should add data attribute for referencing original title", function ( assert ) {
        var tooltip = $('<a href="#" rel="tooltip" title="Another tooltip"></a>').tooltip()
        assert.equal(tooltip.attr('data-original-title'), 'Another tooltip', 'original title preserved in data attribute')
      })
      ...
      test("tooltip title test #1", function ( assert ) {
      ...
      test("tooltip title test #2", function (assert ) {
        var tooltip = $('<a href="#" rel="tooltip" title="Simple tooltip" style="display: inline-block; position: absolute; top: 0; left: 0;"></a>')
          .appendTo('#qunit-fixture')
          .tooltip({
          ...
          .tooltip('show')
        assert.equal($('.tooltip').children('.tooltip-inner').text(), 'Simple tooltip', 'title is set from title attribute while prefered over title option')
        tooltip.tooltip('hide')
        ok(!$(".tooltip").length, 'tooltip removed')
      })

      test("tooltip title test #3", function (assert ) {
